...
refs_2016 = pd.read_csv('https://raw.githubusercontent.com/jlo-dcjd/juv-dashboard/main/apps/data/Referrals_2016_2021.csv', index_col='Referral_Date')
...
def app():
    ...
    fig.add_trace(
        go.Bar(x=months, y=percentage_change(det_perc_df()[year1], det_perc_df()[year2]),
    ...
    figf.add_trace(
        go.Bar(x=monthsf, y=percentage_change(det_perc_df_fy()[year1], det_perc_df_fy()[year2]),
               name='% Change {} vs {}'.format(year1, year2)),
        row=2, col=1
    )
    ...
    fig2 = ex.bar(refs['General_Category'].value_counts(), x=refs['General_Category'].value_counts().index,
                  y=refs['General_Category'].value_counts().values,
